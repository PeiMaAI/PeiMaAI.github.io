...
- **提示检索器**：Rubin等人提出了一种训练提示检索器的方法，利用语言模型生成的数据，为生成器提供更好的示例用于上下文学习，进一步提升了生成质量。
...
在基于LLM的生成过程中，检索操作通常用于补充知识，以增强生成内容的准确性。然而，检索并非总是必要，且过度检索可能引入不相关信息，导致生成错误。因此，如何确定检索的必要性和频率，成为实现鲁棒RAG模型的关键。
